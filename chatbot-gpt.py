...
class GPTChatbot:
    """Main chatbot class using OpenAI GPT API"""

    def __init__(self, api_key: str, knowledge_base: HybridKnowledgeBase,
                 model: str = "gpt-4o-mini", use_compression: bool = True):
    ...
    def setup_gpt(self):
        ...
        try:
            ...
            print(f"✅ เชื่อมต่อ OpenAI GPT API สำเร็จ! (Model: {self.model})")
        except Exception as e:
            print(f"❌ เกิดข้อผิดพลาดในการเชื่อมต่อ OpenAI GPT API: {e}")
            raise
    ...
    def expand_query(self, query: str) -> str:
        ...
        # ⚠️ สำคัญ: ถ้าถามแค่ "กี่ปี" อย่างเดียว ไม่ต้องขยายคำเยอะ เพื่อให้เจอข้อมูลที่สั้นและตรงประเด็น
...
[ข้อความที่สกัดออกมา (ถ้า YES)]"""
    ...
    def create_prompt(self, user_question: str) -> List[Dict[str, str]]:
        """Create a comprehensive prompt with context in ChatGPT message format"""
        ...
        # Create system and user messages for GPT
        system_message = """คุณคือ "น้องบอท" ผู้ช่วยให้คำปรึกษาเกี่ยวกับหลักสูตรและการรับสมัครนักศึกษา
...
- Facebook: คณะครุศาสตร์อุตสาหกรรม มจพ."""
        ...
        user_message = f"""**ข้อมูลอ้างอิง (ตอบได้เฉพาะจากข้อมูลนี้เท่านั้น):**
...
- ถ้าไม่มีข้อมูล ให้บอกตรงๆ และแนะนำช่องทางติดต่อ"""
        ...
        # Return messages in the format GPT expects
        return [
    ...
    def chat(self, user_input: str) -> str:
        ...
        try:
            ...
            messages = self.create_prompt(user_input)
...
def main():
    ...
    if not api_key:
        ...
        print("💡 กรุณาเพิ่ม OPENAI_API_KEY=sk-... ในไฟล์ .env")
        return
    ...
    try:
        ...
        chatbot = GPTChatbot(api_key, kb, model="gpt-4o-mini")
        
        print("=" * 80)
        print("🤖 ยินดีต้อนรับสู่ระบบแชทบอทคณะครุศาสตร์อุตสาหกรรม (GPT Version)")
