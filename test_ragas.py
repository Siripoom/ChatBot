"""
RAGAS Evaluation Script for Chatbot v04
========================================
This script evaluates the RAG chatbot using RAGAS metrics:
...
- Answer Correctness: Combination of similarity and factual correctness
"""
...
try:
    ...
    print("❌ กรุณาติดตั้ง ragas library ด้วยคำสั่ง: pip install ragas langchain-openai")
    exit(1)


class RAGASEvaluator:
    """Class for evaluating RAG chatbot using RAGAS metrics"""

    def __init__(self, chatbot: TyphoonChatbot, openai_api_key: str):
        """
        Initialize RAGAS evaluator

        Args:
            chatbot: TyphoonChatbot instance to evaluate
            openai_api_key: OpenAI API key for RAGAS evaluation (RAGAS requires OpenAI)
        """
        self.chatbot = chatbot
        self.openai_api_key = openai_api_key

        # Initialize OpenAI for RAGAS (RAGAS uses OpenAI for evaluation)
    ...
    def evaluate(self, test_data: List[Dict], use_all_metrics: bool = True) -> Dict:
        """
        Evaluate the chatbot using RAGAS metrics
        ...
        if use_all_metrics:
            ...
            print("\n📊 Evaluating with all RAGAS metrics...")
        else:
            ...
            print("\n📊 Evaluating with core RAGAS metrics...")
    ...
    def save_results(self, results, output_file: str = "ragas_results.json"):
        """
    ...
    def print_results(self, results):
        ...
        print("• Answer Similarity: Semantic similarity with ground truth (1.0 = identical)")
        print("• Answer Correctness: Overall correctness considering facts (1.0 = perfect)")
        print("-" * 80)
...
def main():
    ...
    openai_api_key = os.getenv('OPENAI_API_KEY')  # RAGAS requires OpenAI API
    ...
    if not openai_api_key:
        print("❌ ไม่พบ OPENAI_API_KEY ใน environment variables")
        print("💡 RAGAS requires OpenAI API for evaluation. Please add OPENAI_API_KEY to .env file")
        return
    ...
    try:
        ...
        print("="*80)
        print("1. Full evaluation (all metrics) - More comprehensive but uses more API calls")
        print("2. Core evaluation (faithfulness, relevancy, recall) - Faster and cheaper")
        print("-"*80)

        choice = input("Choose evaluation mode (1 or 2) [default: 2]: ").strip()
        use_all_metrics = (choice == "1")

        # Run evaluation
        results = evaluator.evaluate(test_data, use_all_metrics=use_all_metrics)
